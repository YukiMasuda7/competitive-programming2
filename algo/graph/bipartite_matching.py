# 2部マッチング問題
# 二部グラフにおいて、各辺が「1対1」で
# 人や物を結ぶように選べる最大の辺集合(ペアの組)を求める問題

# 1. スタートからAグループの全ノードに容量1の辺を張る
# 2. グループAのノードから、グループBのノードへの辺を
#    与えられた条件を満たすように容量1の張る
# 3. Bグループの全ノードからゴールに容量1の辺を張る
# 4. スタートからゴールへの最大フロー(容量は0か1)がマッチングする数

# 最大フローを求めたいグラフを構築する（辺の要素は (辺の始点の番号, 辺の終点の番号, 辺の容量) のタプル）
# Aグループ(len=N)、Bグループ(len=M)、始点、終点のインデックスを連続した整数で扱いたいためインデックスを下のようなルールで定める
# 1-indexの場合
# Aグループのi番目の頂点: i
# Bグループのi番目の頂点: N+i
# 始点: N + M + 1
...
for i in range(N):
    edges.append((2 * N + 1, i + 1, 1))  # 「s → Aグループ」の辺
    edges.append((N + i + 1, 2 * N + 2, 1))  # 「Bグループ → t」の辺
